"""Reproducible Keras optimizer benchmark on standard image datasets."""
...
class VisionBenchmarkConfig:
    """Configuration for the vision optimizer benchmarks."""
...
def _load_dataset(
    ...
    (x_train_full, y_train_full), (x_test, y_test) = loaders[dataset_name]()
...
def _aggregate_results(trials: list[TrialResult]) -> list[dict[str, Any]]:
    ...
    keys = sorted({(trial.dataset, trial.optimizer) for trial in trials})
    for dataset_name, label in keys:
        selected = [
            trial
            for trial in trials
            if trial.dataset == dataset_name and trial.optimizer == label
        ]
        rows.append(
            {
                "dataset": dataset_name,
    ...
    rows.sort(key=lambda row: (row["dataset"], -float(row["mean_test_accuracy"])))
    return rows


def _dataset_shapes(data: dict[str, np.ndarray]) -> dict[str, list[int]]:
    return {
...
def run_vision_optimizer_suite(
    specs: tuple[OptimizerSpec, ...],
    config: VisionBenchmarkConfig | None = None,
) -> dict[str, Any]:
    """Run the vision benchmark suite over the supplied optimizer specs."""
    config = config or VisionBenchmarkConfig()
    datasets: dict[str, dict[str, np.ndarray]] = {
        dataset_name: _load_dataset(dataset_name, config.validation_size)
        ...
        for spec in specs:
            for seed in config.seeds:
                trial_results.append(
                    _run_trial(dataset_name, spec, seed, data, config)
                )

    return {
        "task": "keras_vision_standard",
        "date": date.today().isoformat(),
        "environment": _benchmark_environment(),
        "config": asdict(config),
        "datasets": {
            dataset_name: _dataset_shapes(data)
            for dataset_name, data in datasets.items()
        },
...
def run_benchmark(config: VisionBenchmarkConfig | None = None) -> dict[str, Any]:
    """Run the default vision benchmark suite."""
    return run_vision_optimizer_suite(build_default_optimizer_specs(), config=config)
